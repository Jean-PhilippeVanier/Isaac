...
Animation.prototype.setFlipped = function(flipped) {
  this.flipped = flipped;
};

Animation.prototype.setLooped = function(looped) {
  this.looped = looped;
};

Animation.prototype.addImage = function(imagePath) {
  var image = new Image();
  image.src = imagePath;
  this.imageList.push(image);
};
...
// starts at 0
Animation.prototype.changeRow = function(row) {
...
// starts at 0
Animation.prototype.changeCol = function(col) {
...
// starts at 0
Animation.prototype.changeMinMaxInterval = function(min, max) {
...
Animation.prototype.resetCol = function() {
  this.imageCurrentCol = this.imageAnimationColMin;
};

Animation.prototype.getActualWidth = function() {
  return this.imageList[0].width / this.imageTileColCount * this.scale;
};

Animation.prototype.getActualHeight = function() {
  return this.imageList[0].height / this.imageTileRowCount * this.scale;
};

Animation.prototype.tick = function(ctx, spritePosX, spritePosY) {
  ...
  for (var i = 0; i < this.imageList.length; i++) {
    if (this.imageList[i].complete) {
      ...
      if (this.flipped) {
        ctx.save();
        ctx.translate(
          Math.floor(spritePosX - actualW / 2) + actualW,
